/**
 * 途正英语AI分级测评 - 规则说明页
 * 设计风格：教育温暖风，清晰的规则卡片布局
 * 功能：测评规则说明 + 麦克风权限请求 + 进入测评
 */
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      >
        ...
        <h2 className="flex-1 text-center font-bold text-lg text-foreground pr-10">
          测评说明
        </h2>
      </motion.div>
      ...
      <div className="flex-1 px-6 pb-6 space-y-4">
        ...
        >
          <h3 className="font-bold text-foreground text-base mb-4">测评流程</h3>
          <div className="space-y-4">
            <RuleStep
              icon={<Volume2 className="w-4 h-4" />}
              step="1"
              title="听题"
              desc="AI外教用英语向你提出问题，请仔细听"
            />
            <RuleStep
              icon={<Mic className="w-4 h-4" />}
              step="2"
              title="回答"
              desc="点击麦克风按钮，用英语语音回答问题"
            />
            <RuleStep
              icon={<Brain className="w-4 h-4" />}
              step="3"
              title="AI评估"
              desc="AI根据你的回答自动调整下一题难度"
            />
            <RuleStep
              icon={<CheckCircle2 className="w-4 h-4" />}
              step="4"
              title="获取结果"
              desc="5-8题后系统自动判定你的英语水平级别"
            />
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-2xl p-5 shadow-sm"
        >
          <h3 className="font-bold text-foreground text-base mb-3">注意事项</h3>
        ...
        <motion.div
          ...
          className={`rounded-2xl p-5 shadow-sm border-2 transition-colors ${
            micGranted
              ? "bg-mint-light/30 border-mint"
              : "bg-white border-coral/20"
          }`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                micGranted ? "bg-mint/20 text-mint-dark" : "bg-coral/10 text-coral"
              }`}
            >
            ...
            {!micGranted && (
              <Button
                ...
                disabled={requesting}
                size="sm"
                className="bg-coral hover:bg-coral-dark text-white rounded-xl text-xs px-4 shrink-0"
              >
                {requesting ? "请求中..." : "授权"}
              </Button>
            )}
        ...
        >
          <Button
            onClick={handleStart}
            disabled={!micGranted}
            className="w-full h-14 rounded-2xl bg-coral hover:bg-coral-dark text-white text-lg font-bold shadow-lg shadow-coral/30 transition-all duration-300 active:scale-[0.98] disabled:opacity-50 disabled:shadow-none"
          >
...
function RuleStep({
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="w-8 h-8 rounded-full bg-coral/10 flex items-center justify-center text-coral shrink-0 mt-0.5">
...
function TipItem({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <p className="text-warm-gray text-xs">{text}</p>
    </div>
  );
}
