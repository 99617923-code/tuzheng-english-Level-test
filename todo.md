...
- [ ] 修复AI外教语音无法播放（听不到音频）
- [ ] 修复录音录不到音问题（点击录音无效）
- [ ] 修复总是0分说没答案问题（evaluate接口返回异常）

## 测评页UI和交互全面修复
- [ ] 话筒图标改为白色（录音按钮中间的mic图标）
- [ ] 删除"点击播放题目"按钮（listening阶段的手动播放按钮）
- [ ] 音频自动播放（进入题目后自动播放外教语音，无需手动点击）
- [ ] 隐藏英文文本内容（听力对话测试，不显示questionText）
- [ ] 录音改为tap切换模式（点击开始录音/点击结束录音，替代touchstart/touchend）
- [ ] 增加后端Whisper转写降级（同声传译插件失败时用后端转写）
- [ ] 增加TTS降级方案（audioUrl为空时用后端TTS生成语音）

## 重测+确认最终评级机制
- [ ] 结果页增加"重新测评"按钮（用户可多次重测直到满意）
- [ ] 结果页增加"确认最终评级"按钮（用户确认后才显示完整报告和群二维码）
- [ ] 确认前：只显示等级和分数概览，不显示群二维码
- [ ] 确认后：显示完整测评报告+对应级别群二维码，不可更改
- [ ] 确认操作需二次确认弹窗（提示确认后不可更改）
- [ ] 题号从第1题开始（不应一来就第2题）
- [ ] 最少答10题才定级（前10题内答得不好也继续测试，答得好一直升级，至少10题后才定级）
